...
@export var InitialState: WeaponState             #export means you can change it in editor, this var type is a TestState Node

var current_state: WeaponState=null               #start with a null current state, which is also a TestState var
...
func _ready() -> void:                                              #_ready is the first function that runs
	for child in get_children():                                    #gets all states which will be children Nodes in the scene to the TestMachine node
		if child is WeaponState:                                      #checks if  children are test states
